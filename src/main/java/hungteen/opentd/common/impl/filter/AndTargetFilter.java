 ...
 **/
public record AndTargetFilter(List<Holder<ITargetFilter>> filters) implements ITargetFilter {

    public static final Codec<AndTargetFilter> CODEC = RecordCodecBuilder.<AndTargetFilter>mapCodec(instance -> instance.group(
            OTDTargetFilters.getCodec().listOf().fieldOf("filters").forGetter(AndTargetFilter::filters)
    ).apply(instance, AndTargetFilter::new)).codec();

    @Override
    public boolean match(ServerLevel level, Entity owner, Entity target) {
        return this.filters().stream().allMatch(l -> l.get().match(level, owner, target));
    }
